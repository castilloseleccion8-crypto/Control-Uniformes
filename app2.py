...
def generar_pdf(sucursal, fecha, df_para_pdf):
    ...
    pdf.cell(190, 10, "ACUSE DE RECIBO - CARGA DE TALLES (VERANO)", ln=True, align="C")
    ...
    cols_display = ["Empleado", "Pantalon", "Chomba", "Camisa H.", "Camisa M."]
    cols_reales = ["APELLIDO Y NOMBRE", "PANTALON", "CHOMBA", "CAMISA HOMBRE", "CAMISA MUJER"]
...
st.markdown('<div class="subtitulo">UNIFORME VERANO</div>', unsafe_allow_html=True)
...
if password == f"{sucursal_sel.lower().replace(' ', '')}2026":
    ...
    # Columnas de prendas según tu Excel real (uniforme de VERANO: sin camperas)
    prendas = ["PANTALON", "CHOMBA", "CAMISA HOMBRE", "CAMISA MUJER"]
    ...
    if st.button("GUARDAR Y REGISTRAR"):
        try:
            with st.spinner("Sincronizando con la base de datos..."):
                ...
                st.download_button("DESCARGAR ACUSE PDF", pdf_bytes, f"Acuse_Verano_{sucursal_sel}.pdf", "application/pdf")
                st.balloons()
        except Exception:
